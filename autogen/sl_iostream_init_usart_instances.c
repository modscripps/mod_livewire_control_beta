...
// Include instance config 
 #include "sl_iostream_usart_sbe49_config.h"
...
sl_status_t sl_iostream_usart_init_sbe49(void);

sl_status_t sl_iostream_usart_init_vcom(void);


// Instance(s) handle and context variable 
static sl_iostream_uart_t sl_iostream_sbe49;
sl_iostream_t *sl_iostream_sbe49_handle = &sl_iostream_sbe49.stream;
sl_iostream_uart_t *sl_iostream_uart_sbe49_handle = &sl_iostream_sbe49;
static sl_iostream_usart_context_t  context_sbe49;
static uint8_t  rx_buffer_sbe49[SL_IOSTREAM_USART_SBE49_RX_BUFFER_SIZE];
sl_iostream_instance_info_t sl_iostream_instance_sbe49_info = {
  .handle = &sl_iostream_sbe49.stream,
  .name = "sbe49",
  .type = SL_IOSTREAM_TYPE_UART,
  .periph_id = SL_IOSTREAM_USART_SBE49_PERIPHERAL_NO,
  .init = sl_iostream_usart_init_sbe49,
};
...
{
  sl_status_t status;
  USART_InitAsync_TypeDef init_sbe49 = USART_INITASYNC_DEFAULT;
  init_sbe49.baudrate = SL_IOSTREAM_USART_SBE49_BAUDRATE;
  init_sbe49.parity = SL_IOSTREAM_USART_SBE49_PARITY;
  init_sbe49.stopbits = SL_IOSTREAM_USART_SBE49_STOP_BITS;
#if (_SILICON_LABS_32B_SERIES > 0)
#if (SL_IOSTREAM_USART_SBE49_FLOW_CONTROL_TYPE != uartFlowControlSoftware)
  init_sbe49.hwFlowControl = SL_IOSTREAM_USART_SBE49_FLOW_CONTROL_TYPE;
#else
  init_sbe49.hwFlowControl = usartHwFlowControlNone;
#endif
#endif
  sl_iostream_usart_config_t config_sbe49 = { 
    .usart = SL_IOSTREAM_USART_SBE49_PERIPHERAL,
    .clock = SL_IOSTREAM_USART_CLOCK_REF(SL_IOSTREAM_USART_SBE49_PERIPHERAL_NO),
    .tx_port = SL_IOSTREAM_USART_SBE49_TX_PORT,
    .tx_pin = SL_IOSTREAM_USART_SBE49_TX_PIN,
    .rx_port = SL_IOSTREAM_USART_SBE49_RX_PORT,
    .rx_pin = SL_IOSTREAM_USART_SBE49_RX_PIN,
#if (_SILICON_LABS_32B_SERIES > 0)
#if defined(SL_IOSTREAM_USART_SBE49_CTS_PORT)
    .cts_port = SL_IOSTREAM_USART_SBE49_CTS_PORT,
    .cts_pin = SL_IOSTREAM_USART_SBE49_CTS_PIN,
#endif
#if defined(SL_IOSTREAM_USART_SBE49_RTS_PORT)
    .rts_port = SL_IOSTREAM_USART_SBE49_RTS_PORT,
    .rts_pin = SL_IOSTREAM_USART_SBE49_RTS_PIN,
#endif
#endif
#if defined(GPIO_USART_ROUTEEN_TXPEN)
    .usart_index = SL_IOSTREAM_USART_SBE49_PERIPHERAL_NO,
#elif defined(USART_ROUTEPEN_RXPEN)
    .usart_tx_location = SL_IOSTREAM_USART_SBE49_TX_LOC,
    .usart_rx_location = SL_IOSTREAM_USART_SBE49_RX_LOC,
#if defined(SL_IOSTREAM_USART_SBE49_CTS_PORT)
    .usart_cts_location = SL_IOSTREAM_USART_SBE49_CTS_LOC,
#endif
#if defined(SL_IOSTREAM_USART_SBE49_RTS_PORT)
    .usart_rts_location = SL_IOSTREAM_USART_SBE49_RTS_LOC,
#endif
#else
    .usart_location = SL_IOSTREAM_USART_SBE49_ROUTE_LOC,
#endif
  };
  sl_iostream_uart_config_t uart_config_sbe49 = {
    .tx_irq_number = SL_IOSTREAM_USART_TX_IRQ_NUMBER(SL_IOSTREAM_USART_SBE49_PERIPHERAL_NO),
    .rx_irq_number = SL_IOSTREAM_USART_RX_IRQ_NUMBER(SL_IOSTREAM_USART_SBE49_PERIPHERAL_NO),
    .rx_buffer = rx_buffer_sbe49,
    .rx_buffer_length = SL_IOSTREAM_USART_SBE49_RX_BUFFER_SIZE,
    .lf_to_crlf = SL_IOSTREAM_USART_SBE49_CONVERT_BY_DEFAULT_LF_TO_CRLF,
    .rx_when_sleeping = SL_IOSTREAM_USART_SBE49_RESTRICT_ENERGY_MODE_TO_ALLOW_RECEPTION,
#if defined(SL_IOSTREAM_USART_SBE49_FLOW_CONTROL_TYPE)
#if (SL_IOSTREAM_USART_SBE49_FLOW_CONTROL_TYPE == uartFlowControlSoftware)
  ...
  status = sl_iostream_usart_init(&sl_iostream_sbe49,
                                  &uart_config_sbe49,
                                  &init_sbe49,
                                  &config_sbe49,
                                  &context_sbe49);
...
// SBE49 IRQ Handler
void SL_IOSTREAM_USART_TX_IRQ_HANDLER(SL_IOSTREAM_USART_SBE49_PERIPHERAL_NO)(void)
{
  sl_iostream_usart_irq_handler(sl_iostream_sbe49.stream.context);
}

void SL_IOSTREAM_USART_RX_IRQ_HANDLER(SL_IOSTREAM_USART_SBE49_PERIPHERAL_NO)(void)
{
  sl_iostream_usart_irq_handler(sl_iostream_sbe49.stream.context);
}
...
sl_power_manager_on_isr_exit_t sl_iostream_usart_sbe49_sleep_on_isr_exit(void)
{
  return sl_iostream_uart_sleep_on_isr_exit(&sl_iostream_sbe49);
}
                           ...
                           sl_power_manager_em_t to)
{
  ...
  if (((from == SL_POWER_MANAGER_EM2) 
	...
	out = GPIO_PinOutGet(SL_IOSTREAM_USART_SBE49_TX_PORT, SL_IOSTREAM_USART_SBE49_TX_PIN);
	GPIO_PinModeSet(SL_IOSTREAM_USART_SBE49_TX_PORT, SL_IOSTREAM_USART_SBE49_TX_PIN, gpioModePushPull, out);
	...
	GPIO_PinModeSet(SL_IOSTREAM_USART_VCOM_TX_PORT, SL_IOSTREAM_USART_VCOM_TX_PIN, gpioModePushPull, out);
      ...
      out = GPIO_PinOutGet(SL_IOSTREAM_USART_SBE49_TX_PORT, SL_IOSTREAM_USART_SBE49_TX_PIN);
      GPIO_PinModeSet(SL_IOSTREAM_USART_SBE49_TX_PORT, SL_IOSTREAM_USART_SBE49_TX_PIN, gpioModeDisabled, out);
